    ...
    <Card className="border-[var(--tenant-line)] bg-[var(--tenant-surface)] text-[var(--tenant-surface-foreground)]">
      <CardContent className="p-8 sm:p-10">
        ...
        <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2"><Label htmlFor="member_gender">Como deseja ser identificado?</Label><select id="member_gender" value={gender} onChange={(e) => setGender(e.target.value as Gender)} className="flex h-9 w-full rounded-md border border-[var(--tenant-line)] bg-[var(--tenant-surface)] px-3 text-sm text-[var(--tenant-surface-foreground)]"><option value="neutral">Forma neutra</option><option value="masculine">Masculino</option><option value="feminine">Feminino</option></select></div>
          {isLawyer ? <>
