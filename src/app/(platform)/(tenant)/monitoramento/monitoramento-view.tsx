      ...
      <Card>
        <CardContent className="space-y-4 p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              {[
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium transition-colors ${
                    view === value ? "bg-card text-[var(--color-card-foreground)] shadow-sm" : "text-muted-foreground hover:text-foreground"
                  }`}
                >
            ...
            <label className="flex min-w-[260px] flex-1 items-center gap-2 rounded-md border border-border bg-background px-3 py-2 text-sm text-muted-foreground md:max-w-md">
          ...
          {view === "kanban" ? (
            <div className="grid gap-4 xl:grid-cols-4">
              {grouped.map((column) => (
                <div key={column.status} className="min-h-[360px] rounded-lg border border-border bg-muted/35 p-3">
                  <div className="mb-3">
                    <div className="flex items-center justify-between gap-2">
                      <h2 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{column.label}</h2>
                      <span className="rounded-full bg-secondary px-2 py-0.5 font-mono text-xs">{column.processes.length}</span>
                    </div>
