...
/**
 * Detecta cert. A1 (e-CPF A1) no Windows Cert Store.
 * Verifica CurrentUser\My E LocalMachine\My — certificados instalados
 * "para todos os usuários" (comum quando o TI instala via política de
 * grupo) só aparecem no segundo. O Chromium usa os dois automaticamente
 * pra TLS/mTLS; se checássemos só CurrentUser, o diagnóstico dava falso
 * negativo mesmo com o login funcionando de verdade no navegador.
 * Retorna CertA1Info (found: false se não achou ou deu erro).
 */
export function detectarCertA1(): CertA1Info {
  const start = Date.now();
  try {
    logger.info('Detectando cert. A1 no Windows Cert Store (CurrentUser + LocalMachine)...');

    // PowerShell que lista certs com private key, não expirados, nos dois stores.
    // LocalMachine pode falhar silenciosamente sem privilégio elevado — não é
    // fatal, o CurrentUser continua sendo verificado normalmente.
    const psCommand = `
      $certs = @()
    ...
    if (!output || !output.trim() || output.trim() === 'null') {
      ...
      return {
        found: false,
        error: 'Nenhum certificado com chave privada encontrado em Cert:\\CurrentUser\\My nem Cert:\\LocalMachine\\My',
      };
    }
 ...
 * CurrentUser + LocalMachine — ver comentário em detectarCertA1().
 */
