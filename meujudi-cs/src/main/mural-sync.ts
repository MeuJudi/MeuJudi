...
type MuralRequest = { id: string; process_id: string; cnj: string | null; data_inicio: string; data_fim: string };
...
export class MuralSync {
  ...
  private async processPendingRequest(headers: Record<string, string>, request: MuralRequest) {
    ...
    try {
      if (!request.cnj) throw new Error('Processo da solicitação não encontrado.');
      const oabs = await this.getOabs(headers);
      const targetCnj = request.cnj.replace(/\D/g, '');
      ...
      for (const oab of oabs) {
        for (let page = 1; page <= MAX_PAGES_PER_CHUNK; page += 1) {
          const response = await this.mural.buscarPorOAB(oab.oab_number, oab.oab_uf, request.data_inicio, request.data_fim, page);
          const items = response.items ?? [];
          recebidas += items.length;
          const matching = items.filter((item) => item.numero_processo.replace(/\D/g, '') === targetCnj && !seen.has(item.id));
          matching.forEach((item) => seen.add(item.id));
          const result = await this.sendBatch(headers, matching);
          novas += result.novas;
          erros += result.erros;
      ...
      recordDiagnosticEvent('cs_mural_process_request_finished', erros ? 'warning' : 'success', `Consulta pontual concluída: ${request.cnj}`, { requestId: request.id, recebidas, encontradas: seen.size, novas }, Date.now() - startedAt);
    } catch (error) {
      ...
      recordDiagnosticEvent('cs_mural_process_request_failed', 'error', message, { requestId: request.id, cnj: request.cnj }, Date.now() - startedAt);
    }
  }
