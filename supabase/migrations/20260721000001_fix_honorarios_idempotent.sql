-- Correção: garantir que a migration 20260721000000_honorarios_e_oab_cache.sql
-- seja completamente aplicada. O type honorario_unidade pode já existir se a
-- migration anterior foi interrompida.

do $$
begin
  -- Cria o enum se não existir
  if not exists (select 1 from pg_type where typname = 'honorario_unidade') then
    create type public.honorario_unidade as enum (
      'hora',
      'servico',
      'mes',
      'consulta',
